  ...
  return (
    <div className="robotics-home -mt-[76px] overflow-hidden">
      <section className="hero-shell relative flex min-h-[690px] items-center overflow-hidden border-b border-white/10 pt-20 lg:min-h-[760px]">
        ...
        <div className="relative z-10 mx-auto grid w-full max-w-7xl gap-8 px-5 pb-8 pt-20 sm:px-8 sm:pt-24 lg:grid-cols-[1.05fr_.95fr] lg:px-10 lg:pb-10 lg:pt-28">
          <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: .7 }} className="max-w-3xl self-center">
            ...
            <h1 className="max-w-3xl text-[3.2rem] font-semibold leading-[.94] tracking-[-.055em] sm:text-7xl lg:text-[5.7rem]">We engineer<br />machines that<br /><span className="text-[#ffffff]">move Rwanda.</span></h1>
            ...
            <div className="mt-7 flex flex-wrap gap-3"><Link to="/projects" className="robot-button bg-[#ffffff] text-[#06100b] hover:bg-white">Explore our builds <ArrowRight className="h-4 w-4" /></Link><Link to="/register" className="robot-button border border-white/25 bg-white/5 text-white hover:border-[#ffffff] hover:text-[#ffffff]">Join the club</Link></div>
          </motion.div>
          <div className="hidden items-end justify-end lg:flex"><div className="mb-5 w-72 border-l border-[#ffffff]/60 bg-[#07111d]/70 p-5 backdrop-blur-md"><div className="flex items-center justify-between font-mono text-[10px] uppercase tracking-[.2em] text-[#ffffff]">System live <Radio className="h-4 w-4 animate-pulse" /></div><div className="mt-5 grid grid-cols-2 gap-4"><div><b className="text-2xl">ROS2</b><p className="text-sm text-slate-500">Control stack</p></div><div><b className="text-2xl">360°</b><p className="text-sm text-slate-500">LiDAR scan</p></div></div></div></div>
          <div className="col-span-full grid border-y border-white/10 bg-[#07111d]/70 backdrop-blur sm:grid-cols-3">{[['30+', 'active builders'], ['15+', 'working prototypes'], ['04', 'engineering squads']].map(([value, label]) => <div key={label} className="flex items-center gap-4 border-white/10 px-6 py-5 sm:border-r last:border-r-0"><b className="font-mono text-2xl text-[#ffffff]">{value}</b><span className="text-xs uppercase tracking-[.17em] text-slate-400">{label}</span></div>)}</div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-5 py-24 sm:px-8 lg:px-10 lg:py-32">
        ...
        <div className="mt-14 grid border-y border-white/10 md:grid-cols-3">{disciplines.map((item, i) => <motion.article key={item.title} {...reveal} transition={{ duration: .55, delay: i * .12 }} whileHover={{ y: -10 }} className="discipline-card group min-h-[310px] border-white/10 p-8 md:border-r last:border-r-0"><span className="font-mono text-xs text-slate-600">/0{i + 1}</span><item.icon className="mt-12 h-9 w-9 text-[#ffffff]" strokeWidth={1.5} /><h3 className="mt-7 text-2xl font-semibold">{item.title}</h3><p className="mt-4 leading-7 text-slate-400">{item.copy}</p></motion.article>)}</div>
      </section>
      ...
      <section className="mx-auto grid max-w-7xl gap-14 px-5 py-24 sm:px-8 lg:grid-cols-[.8fr_1.2fr] lg:px-10 lg:py-32">
        ...
        <div className="divide-y divide-white/10 border-y border-white/10">{upcoming.map((event, i) => <motion.article key={event.id} initial={{ opacity: 0, x: 45 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: '-60px' }} transition={{ duration: .55, delay: i * .12 }} className="grid gap-5 py-7 sm:grid-cols-[110px_1fr_auto] sm:items-center"><div className="font-mono text-xs uppercase text-[#ffffff]"><CalendarDays className="mb-2 h-5 w-5" />{event.date}</div><div><h3 className="text-xl">{event.title}</h3><p className="mt-2 flex gap-2 text-sm text-slate-500"><MapPin className="h-4 w-4" />{event.location}</p></div><Link to="/events" className="robot-icon"><ArrowRight className="h-4 w-4" /></Link></motion.article>)}</div>
      </section>

      <motion.section initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ duration: .7 }} className="cta-section bg-[#ffffff] px-5 py-20 text-[#06100b]"><div className="mx-auto flex max-w-7xl flex-col gap-8 lg:flex-row lg:items-center lg:justify-between"><div><p className="font-mono text-xs font-bold uppercase tracking-[.2em]">Applications open</p><h2 className="mt-4 text-4xl font-semibold sm:text-6xl">Your first robot starts here.</h2></div><Link to="/register" className="robot-button self-start bg-[#06100b] text-white hover:bg-white hover:text-[#06100b]">Request membership <ArrowRight className="h-4 w-4" /></Link></div></motion.section>
    </div>
  )
}