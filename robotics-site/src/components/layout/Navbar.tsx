    ...
    <header className="robot-navbar sticky top-0 z-50 flex justify-center border-b px-4 backdrop-blur-xl sm:px-6">
      <nav className="mx-auto flex w-full max-w-7xl items-center justify-between px-1 py-3 sm:px-2">
        ...
        <div className="hidden items-center gap-1 xl:flex">
          ...
          <Link to="/register" className="ml-2 bg-[#ffffff] px-4 py-2.5 text-sm font-bold text-[#06100b] hover:bg-white">Join club</Link>
        </div>
      ...
      {open && <div className="absolute left-0 right-0 top-full z-40 px-3 pt-2 xl:hidden"><div className="robot-mobile-menu mx-auto max-w-7xl space-y-2 border p-4 shadow-xl backdrop-blur-xl">{navLinks.map(link => <NavLinkItem key={link.path} path={link.path} label={link.label} onClick={() => setOpen(false)} />)}<Link to="/register" className="block w-full bg-[#ffffff] px-4 py-3 text-center text-sm font-bold text-[#06100b]" onClick={() => setOpen(false)}>Join club</Link></div></div>}
    </header>
  )
}